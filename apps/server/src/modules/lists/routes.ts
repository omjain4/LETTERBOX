    ...
    try {
        ...
        if (position === undefined) {
            const lastItem = await prisma.listItem.findFirst({
                where: { listId: String(String(req.params.id)) },
        ...
        const item = await prisma.listItem.create({
            data: {
                listId: String(String(req.params.id)),
...
router.delete("/:id", async (req: AuthRequest, res: Response) => {
    ...
    await prisma.list.delete({ where: { id: String(String(req.params.id)) } });
    res.status(204).send();
});
...
router.delete("/:listId/items/:itemId", async (req: AuthRequest, res: Response) => {
    const list = await prisma.list.findFirst({
        where: { id: String(String(req.params.listId)), userId: req.userId },
    });
    if (!list) {
        res.status(404).json({ error: "List not found" });
        return;
    ...
    await prisma.listItem.delete({ where: { id: String(String(req.params.itemId)) } });
    res.status(204).send();
});
